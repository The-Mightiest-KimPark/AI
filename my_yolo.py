...
#         thickness = (image.size[0] + image.size[1]) // 300
        ...
        coords = []
#         print(out_boxes)
#         x_coords = [(lefts[i] + rights[i])/2 for i in range(len(out_classes))]
#         y_coords = [(tops[i] + bottoms[i])/2 for i in range(len(out_classes))]
        ...
        my_result = [{'name' : name_unq[j], 'count': 0, 'coord' : [] } for j in range(name_unq.shape[0])]

        for i in range(df.shape[0]):
            for j in range(len(my_result)):
                if my_result[j]['name'] == df.iloc[i,0]:
                    my_result[j]['count'] += 1
                    my_result[j]['coord'].append([df.iloc[i,:][1] ,df.iloc[i,:][2]])
                    break
